...
export class App implements OnInit {
  ...
  async getMembers() {
    // Returning a promise
    try {
      return lastValueFrom(this.http.get<User[]>('https://localhost:5001/api/members')); // Can be also firstValue from since there is only one argument which is the response
    } catch (error) {
      console.log(error);
      throw error;
    }
